...
(function () {
  ...
  const SNAP = window.Snap2Map || {};
  const calibrator = SNAP.calibrator;

  if (!calibrator) {
    // eslint-disable-next-line no-console
    console.error('Snap2Map core modules are missing.');
    return;
  }
  ...
  function updateStatusText() {
    ...
    if (state.lastPosition) {
      const ring = calibrator.computeAccuracyRing(state.calibration, state.lastPosition.coords.accuracy || 50);
      if (ring) {
  ...
  function updateLivePosition() {
    ...
    const pixel = calibrator.projectLocationToPixel(state.calibration, location);
    if (!pixel) {
      return;
    }
    ...
    const ring = calibrator.accuracyRingRadiusPixels(state.calibration, location, coords.accuracy || 50);
  ...
  function recalculateCalibration() {
    ...
    const result = calibrator.calibrateMap(state.pairs);
